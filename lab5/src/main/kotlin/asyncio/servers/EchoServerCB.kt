...
class EchoServerCB(private val port: Int) {
    ...
    fun run() {
        ...
        logger.info("provider: ${servSocket.provider().javaClass.name}")
        servSocket.bind(InetSocketAddress("0.0.0.0", port))
        var clientId = 1

        fun acceptLoop() {
            servSocket.acceptAsync() { err, client ->
                if (err != null) {
    ...
    fun processClient(client: AsynchronousSocketChannel,  id: Int) {
        ...
        fun bye() {
            putBuffer(byeMsg)
            client.writeAsync(buffer) { err, res ->
                buffer.clear()
        ...
        fun clientLoop() {
            client.readAsync(buffer) { err, nBytes ->
                if (err != null || nBytes < 0 ) {
                    ...
                    if (isExitCmd()) {
                        ...
                        client.writeAsync(buffer) { err, nbytes ->
                            buffer.clear()
                            clientLoop()
                        }
                    }
