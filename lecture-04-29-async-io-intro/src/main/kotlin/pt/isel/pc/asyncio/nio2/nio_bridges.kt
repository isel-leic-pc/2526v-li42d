...
fun AsynchronousServerSocketChannel.acceptAsync(cont: IOContinuation<AsynchronousSocketChannel?>) {
    this.accept(cont, acceptCompleted )
}

fun AsynchronousSocketChannel.readAsync (dst: ByteBuffer, cont: IOContinuation<Int>) {
    this.read(dst, cont, rwCompleted)
}


fun AsynchronousSocketChannel.writeAsync (src: ByteBuffer, cont: IOContinuation<Int>) {
...
fun AsynchronousSocketChannel.connectAsync(endPoint: InetSocketAddress) : CompletableFuture<Void> {
...
fun AsynchronousSocketChannel.readAsync (dst: ByteBuffer ) : CompletableFuture<Int>{
